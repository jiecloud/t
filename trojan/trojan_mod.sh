...
#Author: atrandys
#Mod by cnrock
#change  port 80/ 443 to 8080/ 4430
#
#
function blue(){
...
Port8080=`netstat -tlpn | awk -F '[: ]+' '$1=="tcp"{print $5}' | grep -w 8080`
Port4430=`netstat -tlpn | awk -F '[: ]+' '$1=="tcp"{print $5}' | grep -w 4430`
if [ -n "$Port8080" ]; then
    process80=`netstat -tlpn | awk -F '[: ]+' '$5=="8080"{print $9}'`
    red "==========================================================="
    red "检测到8080端口被占用，占用进程为：${process8080}，本次安装结束"
    red "==========================================================="
    exit 1
fi
if [ -n "$Port4430" ]; then
    process4430=`netstat -tlpn | awk -F '[: ]+' '$5=="4430"{print $9}'`
    red "============================================================="
    red "检测到4430端口被占用，占用进程为：${process4430}，本次安装结束"
    red "============================================================="
    exit 1
fi
if [ -f "/etc/selinux/config" ]; then
    ...
    if [ "$CHECK" != "SELINUX=disabled" ]; then
        green "检测到SELinux开启状态，添加放行8080/4430端口规则"
        yum install -y policycoreutils-python >/dev/null 2>&1
        semanage port -m -t http_port_t -p tcp 8080
        semanage port -m -t http_port_t -p tcp 4430
    fi
fi
if [ "$release" == "centos" ]; then
    ...
    if [ -n "$firewall_status" ]; then
        green "检测到firewalld开启状态，添加放行8080/4430端口规则"
        firewall-cmd --zone=public --add-port=8080/tcp --permanent
	firewall-cmd --zone=public --add-port=4430/tcp --permanent
	firewall-cmd --reload
    fi
...
Port8080=`netstat -tlpn | awk -F '[: ]+' '$1=="tcp"{print $5}' | grep -w 8080`
if [ -n "$Port8080" ]; then
    process8080=`netstat -tlpn | awk -F '[: ]+' '$5=="8080"{print $9}'`
    red "==========================================================="
    red "检测到8080端口被占用，占用进程为：${process8080}，本次安装结束"
    red "==========================================================="
    exit 1
...
start_menu(){
    ...
    blue " 声明："
    red " *请不要在任何生产环境使用此脚本"
    red " *请不要有其他程序占用8080和4430端口"
    red " *如果安装了v2ray,请先停止，acme.sh申请新证书时需要80端口"
